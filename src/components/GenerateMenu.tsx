...
export const GenerateVerticalMenu = ({ menuData }: { menuData: VerticalMenuDataType[] }) => {
  ...
  const renderMenuItems = (data: VerticalMenuDataType[]) => {
    ...
    return data.map((item: VerticalMenuDataType, index) => {
      ...
      const icon = typeof menuItem.icon === 'string' ? <i className={menuItem.icon} /> : menuItem.icon

      // Check if the current item is a section
      if ('isSection' in menuSectionItem) {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const { children, isSection, ...rest } = menuSectionItem as VerticalSectionDataType
